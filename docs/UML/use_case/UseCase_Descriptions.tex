...
\newcommand{\ucphase}[1]{%
    \noindent\textbf{\color{helixpurplemid}Phase:}\quad%
    \ifstrequal{#1}{PoC}{\textcolor{helixteal}{\textbf{PoC}}}{\textcolor{helixamber}{\textbf{Future MVP}}}%
    \par\medskip
}
...
\begin{center}
    \begin{tabular}{C{1.0cm} L{3.2cm} L{6.5cm}}
        ...
        A8 & AI Service & Internal system actor; provides OpenAI-powered responses \\
        \bottomrule
    \end{tabular}
\end{center}
...
\begin{enumerate}
    ...
    \item File Integrity Monitoring (UC-34 to UC-36)
\end{enumerate}
...
The Detection Engine pipeline follows a flat inclusion model: UC-11 (Automated Detection) \texttt{<<include>>}
UC-12 (Ingest Logs), UC-13 (Pre-filter Noise), UC-14 (Score Anomalies), and UC-15 (Generate Alerts) as
direct sub-use cases. UC-14 in turn \texttt{<<include>>} UC-16 (Use ML Scoring). UC-15 does NOT include
UC-14 -- it consumes the scored output as a precondition, avoiding diamond inclusion.
...
\begin{ucbox}{UC-11 \quad Automated Detection}
    ...
    \begin{enumerate}
        ...
        \item \texttt{<<include>>} UC-13 -- Pre-filter Noise (processes ingested entries).
        \item \texttt{<<include>>} UC-14 -- Score Anomalies (includes UC-16 ML Scoring).
        \item \texttt{<<include>>} UC-15 -- Generate Alerts for HIGH/CRITICAL events.
        \item \texttt{<<extend>>} UC-31 -- Use AI Assistant optionally provides alert context enrichment.
        \item System notifies assigned analysts of critical alerts.
    \end{enumerate}
    \textbf{\color{helixpurplemid}Alternatives:}
    \begin{itemize}
        ...
        \item \textbf{[Alt-C]} AI enrichment (\texttt{<<extend>>} UC-31) available as optional post-alert extension point.
    \end{itemize}
\end{ucbox}

\section{UC-12 -- Ingest Logs}
\begin{ucbox}{UC-12 \quad Ingest Logs \textnormal{(included by UC-11)}}
...
\begin{ucbox}{UC-13 \quad Pre-filter Noise \textnormal{(included by UC-11)}}
    \ucfield{Actors}{Detection Engine}
    \ucphase{PoC}
...
\begin{ucbox}{UC-14 \quad Score Anomalies \textnormal{(included by UC-11)}}
    \ucfield{Actors}{Detection Engine}
    \ucphase{PoC}
...
\begin{ucbox}{UC-27 \quad Study with Premium Materials}
    ...
    \begin{itemize}
        ...
        \item \textbf{[Alt-B]} Learner wants notes $\rightarrow$ UC-28 (\texttt{<<extend>>}) is activated.
    \end{itemize}
\end{ucbox}
...
\begin{ucbox}{UC-31 \quad Use AI Assistant}
    ...
    \begin{enumerate}
        ...
        \item \texttt{<<extend>>} UC-33 -- Suggest Attack Methods (Red/Purple Team only).
...
\begin{ucbox}{UC-33 \quad Suggest Attack Methods \textnormal{(\texttt{<<extend>>} UC-31)}}
    \ucfield{Actors}{AI Service (internal)}
    \ucphase{PoC}
    \ucfield{Preconditions}{UC-31 initiated; Red Team or Purple Team role detected.}
    \ucfield{Postconditions}{Relevant attack techniques returned.}
    \textbf{\color{helixpurplemid}Main Scenario:}
    \begin{enumerate}
        \item System detects user role is Red Team or Purple Team.
        \item \texttt{<<extend>>} condition met: AI includes attack technique suggestions.
    ...
    \begin{itemize}
        ...
        \item \textbf{[Alt-B]} User role is not Red/Purple $\rightarrow$ attack suggestions omitted.
    \end{itemize}
\end{ucbox}

% ══════════════════════════════════════════════════════════════════════════
% CHAPTER 8 – FILE INTEGRITY MONITORING
% ══════════════════════════════════════════════════════════════════════════
\chapter{File Integrity Monitoring}

\section{UC-34 -- Create Baseline}
\begin{ucbox}{UC-34 \quad Create Baseline}
    \ucfield{Actors}{Blue Team Operator (primary), Purple Team Operator, Administrator}
    \ucphase{PoC}
    \ucfield{Preconditions}{User authenticated with appropriate role. Target directory path is valid.}
    \ucfield{Postconditions}{Baseline hash snapshot computed and stored for future verification.}
    \textbf{\color{helixpurplemid}Main Scenario:}
    \begin{enumerate}
        \item Operator specifies the target directory path for monitoring.
        \item System invokes the Java module via \texttt{shell\_exec()} to compute SHA-256 hashes.
        \item Baseline record created with file count and hash data.
        \item System confirms baseline creation and displays summary.
    \end{enumerate}
    \textbf{\color{helixpurplemid}Alternatives:}
    \begin{itemize}
        \item \textbf{[Alt-A]} Invalid directory path $\rightarrow$ validation error returned.
        \item \textbf{[Alt-B]} Java module unavailable $\rightarrow$ error logged; operator notified.
    \end{itemize}
\end{ucbox}

\section{UC-35 -- Verify Integrity}
\begin{ucbox}{UC-35 \quad Verify Integrity \textnormal{(\texttt{<<extend>>} UC-34)}}
    \ucfield{Actors}{Blue Team Operator (primary), Purple Team Operator, Administrator}
    \ucphase{PoC}
    \ucfield{Preconditions}{Baseline exists for the target path.}
    \ucfield{Postconditions}{Scan results showing added, modified, deleted, or unchanged files.}
    \textbf{\color{helixpurplemid}Main Scenario:}
    \begin{enumerate}
        \item Operator selects an existing baseline to verify.
        \item System re-computes SHA-256 hashes for the current file state.
        \item System compares against stored baseline and categorises changes.
        \item Results displayed with file paths and change status.
    \end{enumerate}
    \textbf{\color{helixpurplemid}Alternatives:}
    \begin{itemize}
        \item \textbf{[Alt-A]} Baseline not found $\rightarrow$ error returned; create baseline first.
        \item \textbf{[Alt-B]} New files detected $\rightarrow$ flagged as \texttt{added}.
        \item \textbf{[Alt-C]} Files missing $\rightarrow$ flagged as \texttt{deleted}.
    \end{itemize}
\end{ucbox}

\section{UC-36 -- View Scan History}
\begin{ucbox}{UC-36 \quad View Scan History}
    \ucfield{Actors}{Blue Team Operator (primary), Purple Team Operator, Administrator}
    \ucphase{PoC}
    \ucfield{Preconditions}{User authenticated. At least one baseline or scan exists.}
    \ucfield{Postconditions}{Historical scan results displayed; no state change.}
    \textbf{\color{helixpurplemid}Main Scenario:}
    \begin{enumerate}
        \item Operator navigates to the FIM history view.
        \item System lists all baselines and associated scans for the user.
        \item Operator selects a specific scan to view detailed results.
    \end{enumerate}
    \textbf{\color{helixpurplemid}Alternatives:}
    \begin{itemize}
        \item \textbf{[Alt-A]} No history exists $\rightarrow$ empty state with guidance to create baseline.
    \end{itemize}
\end{ucbox}
...
\begin{successbox}[title=Summary]
    This document provides complete textual descriptions for all \textbf{36 use cases}
    (UC-01 to UC-36) of the HELIX platform, fully aligned with the SRS and Use Case Diagram.
    All descriptions follow the academic format: \textit{Name, Actors, Preconditions,
    Postconditions, Main Scenario, and Alternative Scenarios}. The actors
    (Authenticated User, Administrator, Blue Team Operator, Red Team Operator,
    Purple Team Operator, Visitor, Learner, Detection Engine, AI Service) are consistently applied.
    Use case relationships use \texttt{<<include>>} for mandatory sub-flows and
    \texttt{<<extend>>} for conditional extensions.
\end{successbox}
