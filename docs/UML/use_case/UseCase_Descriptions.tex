...
The Detection Engine pipeline is a sequential chain of \texttt{<<include>>} relationships:
UC-11 (Automated Detection) \texttt{<<include>>} UC-12 (Ingest Logs) $\rightarrow$ UC-13 (Pre-filter Noise) $\rightarrow$
UC-14 (Score Anomalies) $\rightarrow$ UC-15 (Generate Alerts).
UC-14 also includes UC-16 (Use ML Scoring).
